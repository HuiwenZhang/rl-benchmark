...
class A2CNetwork(object):
    def __init__(
        ...
        if self.h2_size > 0:
            ...
            # value function
            W_value = tf.Variable(tf.truncated_normal(
                [self.h1_size, 1], stddev=0.01))
            b_value = tf.Variable(0.0 * tf.ones([1]))
            self.value = tf.matmul(h_fc1, W_value) + b_value

            # policy function
            W_policy = tf.Variable(tf.truncated_normal(
                [self.h1_size, self.a_dim], stddev=0.01))
            b_policy = tf.Variable(0.0 * tf.ones([self.a_dim]))

            # prevent problem when calling log(self.policy)
            if self.is_training:
                self.policy = tf.nn.softmax(
                    tf.matmul(h_fc1, W_policy) + b_policy) + 0.00001
        ...
        # Loss functions
        self.value_diff = self.rho_forward * \
        ...
        self.policy_diff = tf.log(
            self.responsible_outputs) * self.advantages * self.weights
        ...
        self.policy_y_one_hot = tf.one_hot(
            self.policy_y, self.a_dim, 1.0, 0.0, name='policy_y_one_hot')

        self.loss_sl = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(
                logits=self.policy,
                labels=self.policy_y_one_hot))
    ...
    def getLoss(
            ...
            weights,
            rho_forward):
        return self.sess.run([self.value_loss,
                              self.policy_loss,
                              self.entropy,
                              self.loss],
    ...
    def train_SL(self, inputs, labels):
        ...
        return self.sess.run([self.loss_combined, self.optimize_sl], feed_dict={
                             self.inputs: inputs, self.policy_y: labels})
